...
export function buildAssessmentPdf(rawAssessment, vehicleDetails, market, identifier, checkDate, bregoData, editLayer) {
  ...
  if (assessment._damageCards?.length > 0) {
    ...
    const lines = assessment._damageCards.filter(c => !struckKeys.has(c._rowKey) && !c._notVisibleFloor).map(c0 => repriceStoredEntry(c0, lampRepriced.get(c0._rowKey))).map(c => {
      ...
      const head = `${c.part} — ${bits.join(', ')}: ${(c._structFloor || c._fromFigure) ? `from ${g(c.cost)}` : (c.cost ? g(c.cost) : '£0')}`;
      return c.note ? `${head}\n  ${c.note}` : head;
    });
  ...
  const disclaimer =
    `Generated by MotorQuoter - motorquoter.app - ${checkDate} - This assessment is AI-generated using the MotorQuoter Assessment Engine and is for bidding guidance only. It is not a professional repair quote. ${REPAIR_FIGURE_FOOTER} Hidden, secondary, or unphotographed damage may increase actual costs. MotorQuoter is not affiliated with Copart, CAP, HPI or any third-party data provider. Always commission a physical inspection before bidding.`;
