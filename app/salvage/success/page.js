      ...
      <div className="app">
        ...
        {status === 'success' && assessment && (
          <>
            ...
            {/* ROI Vehicle Identity */}
            {market === 'IE' && vehicleDetails && (
              <div className="section">
                <div className="section-title">🇮🇪 ROI Vehicle Identity</div>
                <div className="section-body">
                  {vehicleDetails.make && (
                    <div className="field-row">
                      <div className="field-key">Make / Model</div>
                      <div className="field-val">{[vehicleDetails.make, vehicleDetails.model].filter(Boolean).join(' ')}</div>
                    </div>
                  )}
                  {vehicleDetails.year && (
                    <div className="field-row">
                      <div className="field-key">Year of Manufacture</div>
                      <div className="field-val">{vehicleDetails.year}</div>
                    </div>
                  )}
                  {vehicleDetails.colour && (
                    <div className="field-row">
                      <div className="field-key">Colour</div>
                      <div className="field-val">{vehicleDetails.colour}</div>
                    </div>
                  )}
                  {vehicleDetails.fuelType && (
                    <div className="field-row">
                      <div className="field-key">Fuel Type</div>
                      <div className="field-val">{vehicleDetails.fuelType}</div>
                    </div>
                  )}
                  {vehicleDetails.engineSize && (
                    <div className="field-row">
                      <div className="field-key">Engine Size</div>
                      <div className="field-val">{vehicleDetails.engineSize}</div>
                    </div>
                  )}
                  {vehicleDetails.motStatus && (
                    <div className="field-row">
                      <div className="field-key">NCT Status</div>
                      <div className="field-val" style={{ color: vehicleDetails.motStatus === 'Valid' ? '#4ade80' : '#f87171' }}>
                        {vehicleDetails.motStatus}{vehicleDetails.motExpiryDate ? ` · expires ${vehicleDetails.motExpiryDate}` : ''}
                      </div>
                    </div>
                  )}
                  {vehicleDetails.monthOfFirstRegistration && (
                    <div className="field-row">
                      <div className="field-key">First Registered in Ireland</div>
                      <div className="field-val">{vehicleDetails.monthOfFirstRegistration}</div>
                    </div>
                  )}
                </div>
              </div>
            ...
            {/* ROI Bid Intelligence */}
            {market === 'IE' && vehicleDetails?.roiData?.valuation && (
              <div className="section">
                <div className="section-title">🇮🇪 Bid Intelligence</div>
                <div className="section-body">
                  {vehicleDetails.roiData.valuation?.current?.retail && (
                    <div className="field-row">
                      <div className="field-key">Current Retail Value</div>
                      <div className="field-val" style={{ fontFamily: "'Barlow Condensed', sans-serif", fontSize: 18, fontWeight: 800 }}>
                        €{Number(vehicleDetails.roiData.valuation.current.retail).toLocaleString('en-IE')}
                      </div>
                    </div>
                  )}
                  {vehicleDetails.roiData.valuation?.current?.trade && (
                    <div className="field-row">
                      <div className="field-key">Trade Value</div>
                      <div className="field-val">€{Number(vehicleDetails.roiData.valuation.current.trade).toLocaleString('en-IE')}</div>
                    </div>
                  )}
                  {vehicleDetails.roiData.valuation?.future?.retail && (
                    <div className="field-row">
                      <div className="field-key">Future Retail Value</div>
                      <div className="field-val">€{Number(vehicleDetails.roiData.valuation.future.retail).toLocaleString('en-IE')}</div>
                    </div>
                  )}
                  {vehicleDetails.roiData.marketDemand && (() => {
                    const d = vehicleDetails.roiData.marketDemand;
                    const score = d?.demand_score ?? d?.demandScore ?? d?.score;
                    const days = d?.average_days_to_sell ?? d?.avgDaysToSell ?? d?.days_to_sell;
                    return (score || days) ? (
                      <>
                        {score != null && (
                          <div className="field-row">
                            <div className="field-key">Market Demand Score</div>
                            <div className="field-val">{score}</div>
                          </div>
                        )}
                        {days != null && (
                          <div className="field-row">
                            <div className="field-key">Avg Days to Sell</div>
                            <div className="field-val">{days} days</div>
                          </div>
                        )}
                      </>
                    ) : null;
                  })()}
                  {vehicleDetails.roiData.priceGuide && (() => {
                    const pg = vehicleDetails.roiData.priceGuide;
                    const retail = pg?.retail_price ?? pg?.retailPrice ?? pg?.retail;
                    return retail ? (
                      <div className="field-row">
                        <div className="field-key">Cartell Price Guide</div>
                        <div className="field-val">€{Number(retail).toLocaleString('en-IE')}</div>
                      </div>
                    ) : null;
                  })()}
                </div>
            ...
            {/* ROI Risk Flags (History tier) */}
            {market === 'IE' && vehicleDetails?.roiData?.historyCheck && (() => {
              const h = vehicleDetails.roiData.historyCheck;
              const hasFinance = h?.finance_outstanding ?? h?.financeOutstanding ?? h?.outstanding_finance;
              const owners = h?.number_of_owners ?? h?.numberOfOwners ?? h?.owners;
              const stolen = h?.stolen ?? h?.is_stolen;
              if (!h || (hasFinance == null && owners == null && stolen == null)) return null;
              return (
                <div className="section" style={{ borderColor: hasFinance || stolen ? 'rgba(248,113,113,0.4)' : undefined }}>
                  <div className="section-title" style={{ color: hasFinance || stolen ? '#f87171' : undefined }}>
                    {hasFinance || stolen ? '⚠ ' : ''}🇮🇪 Risk Flags — Cartell History
                  </div>
                  <div className="section-body">
                    {stolen != null && (
                      <div className="field-row">
                        <div className="field-key">Stolen Status</div>
                        <div className="field-val" style={{ color: stolen ? '#f87171' : '#4ade80', fontWeight: 700 }}>
                          {stolen ? '⚠ Reported Stolen' : 'No stolen record'}
                        </div>
                      </div>
                    )}
                    {hasFinance != null && (
                      <div className="field-row">
                        <div className="field-key">Outstanding Finance</div>
                        <div className="field-val" style={{ color: hasFinance ? '#f87171' : '#4ade80', fontWeight: 700 }}>
                          {hasFinance ? '⚠ Finance outstanding' : 'No finance recorded'}
                        </div>
                      </div>
                    )}
                    {owners != null && (
                      <div className="field-row">
                        <div className="field-key">Number of Owners</div>
                        <div className="field-val">{owners}</div>
                      </div>
                    )}
                  </div>
                </div>
