...
Live Brego market valuation data is provided in the prompt body when available. Use these figures as the authoritative exit value reference base — they replace any training-memory-derived valuation. Never generate exit values from training memory when live data is provided. If Brego data is marked UNAVAILABLE, state explicitly that live market valuation was not retrieved and produce a wider margin range with Confidence Level: Low.
The mileage source used for Brego valuation is stated in the prompt body. If source is dvsa_mot: the Brego retail figures may be overstated — the vehicle may have accumulated mileage since the last MOT. Apply a 5–10% downward caution adjustment to the Brego retail figures and reduce Confidence Level by one tier (High → Medium, Medium → Low). If source is default_fallback: apply the same downward adjustment and note that mileage was unavailable. If source is copart_listed: the Brego figures are calibrated to current listing condition — use them directly with the standard Cat N/S discount applied. If source is photo_odometer: the Brego figures are calibrated to the dashboard-confirmed mileage read directly from the auction photos — this is the most reliable mileage source. Use the figures directly with the standard Cat N/S discount. Apply NO downward caution adjustment and do NOT reduce confidence on mileage grounds. Never present a Brego valuation based on stale mileage as if it were calibrated to current condition.
...
The Estimated Retail Value is variable in origin — it may be the vendor's own reserve price or a rough market estimate. Cross-reference against the live Brego retail_average (when available) before drawing any inference from the figure. Treat with moderate caution.

Q suffix (Copart purchased to resell):
The Estimated Retail Value is Copart's own commercial target sale price — it is not an insurance settlement and not an independent market valuation. Copart has a direct commercial interest in anchoring bidder expectations upward. This figure is likely above Copart's purchase cost and may be materially above realistic clean retail. When a Q-suffix ERV is present, state in the output: "Copart's Estimated Retail Value of £[X] is a commercial target on a Q-suffix lot, not an insurance settlement. Verify against the live Brego retail_average before using for margin calculation." Treat with significant scepticism.
