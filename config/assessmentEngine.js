...
The mileage source used for Brego valuation is stated in the prompt body. If source is dvsa_mot: the Brego retail figures may be overstated — the vehicle may have accumulated mileage since the last MOT. Apply a 5–10% downward caution adjustment to the Brego retail figures and reduce Confidence Level by one tier (High → Medium, Medium → Low). If source is default_fallback: apply the same downward adjustment and note that mileage was unavailable. If source is copart_listed: the Brego figures are calibrated to current listing condition — use them directly with the standard Cat N/S discount applied. Never present a Brego valuation based on stale mileage as if it were calibrated to current condition.
