...
SCALE-INDEPENDENT — this rule applies at every level: zone, panel, and component. Within a damage zone, separate damage on separate panels must be described and costed independently — do not invent a continuous run to connect them. A dent on the front door and a separate dent on the rear door are two separate repairs, not a continuous sweep across the flank. Do not cost an intervening or adjacent panel because it sits between two damaged ones — cost each panel only on its own visible damage. The exception: if photos show visible continuous deformation connecting panels in the photos, a connected description is correct. Absent visible continuous deformation, separate damage stays separate.
