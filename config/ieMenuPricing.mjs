...
//
// ✅ FX-PIN DUPLICATION RESOLVED (batch 52, 24 Aug): this is the SINGLE source of the GBP/EUR rate.
//   config/vrt.mjs (the import product's FX pin) previously declared its own `GBP_EUR_RATE = 1.17`;
//   it now imports IE_MENU_GBP_EUR_RATE from here and re-exports it under that name, so a repin happens
//   ONCE, here, and turns the gate red (standing rule 5). Both were 1.17 at reconciliation — no figure
//   moved. Keep this a LITERAL, never computed at runtime.
