...
function sheet(info) {
  const { lot, dir, vd, bodyClass, photos, gt, panels } = info;
...
<header><h1>${esc(lot)}</h1><span class="mut">${esc(vd.make)} ${esc(vd.model)} · ${esc(vd.bodyStyle)} · panel set: ${esc(bodyClass)} · ${photos.length} photos</span>
<span class="mut">Label from the photos only. The engine's verdicts are deliberately not shown.</span>
<button type="button" class="save" id="save">Save ground-truth.json</button><a href="index.html">all lots</a></header>
