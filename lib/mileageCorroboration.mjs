...
// batch 177 P5 — "no MOT record because the car is under 3 years old": no DVSA MOT mileage, and fewer than 3 years by
// registration year. Pure and here (not in the route) so the route stays clock-free (validate-prompt-batch118).
export function noMotUnderThree(vehicleYear, dvsaMileagePresent, nowYear = new Date().getFullYear()) {
  const y = parseInt(String(vehicleYear ?? ''), 10);
  return !dvsaMileagePresent && Number.isFinite(y) && (nowYear - y) < 3;
}
...
export function buildMileageCorroborationSlot(enrichedVd, brMileage, brMileageSource) {
  ...
  // batch 177 P5 (Vincent, 22 Sep): a car under 3 years has no MOT record yet, so a second source cannot exist. When the
  // dash photo read equals the listing, say that plainly — still 'unconfirmed' (batch 106 §4: one source), same ask.
  if (enrichedVd._noMotUnder3 === true && enrichedVd._photoOdometerAgrees === true) {
    return buildSlot({
      id: 'mileage-corroboration', label: 'Mileage corroborated against other sources',
      kind: 'confirmation', verdict: 'unconfirmed',
      detail: `${fmtMiles(brMileage)} — the listing and the dashboard photo agree; no MOT record exists yet (car under 3 years), so nothing independent corroborates it.`,
      confidence: 'inferred', source: 'code',
