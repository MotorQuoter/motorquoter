...
export async function getDvsaMotHistory(vrm) {
  ...
  // DVSA sets hasOutstandingRecall=true for "no data" responses, not just confirmed recalls.
  // Clear the flag when the accompanying message is the no-data variant.
  if (data.hasOutstandingRecall && data.recallInformation) {
    const info = Array.isArray(data.recallInformation)
      ? data.recallInformation.join(' ')
      : String(data.recallInformation);
    if (/we do not have recall information|contact your local/i.test(info)) {
      data.hasOutstandingRecall = false;
    }
  }
