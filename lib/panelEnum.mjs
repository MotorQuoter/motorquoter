...
export const PANEL = Object.freeze({
  ...
  WHEEL_ARCH_MOULDING: 'WHEEL_ARCH_MOULDING', // arch trim/surround — standard on crossovers/SUVs; costed ONLY when seen, never paired
  // batch 157 — ENGINE FREEZE (Vincent, 18 Sep). These two are priced, eligible and scored, but DORMANT:
  // batch 156's vocabulary lines were taken back out of the prompt, so the model is never told the names
  // and never writes a row for them. Nothing reaches a customer until the prompt lines go back, and that
  // needs a re-record Vincent has costed and approved. Everything else about them is in place and tested.
  WHEEL_ARCH_LINER:   'WHEEL_ARCH_LINER',   // batch 156 — the plastic/fibre liner INSIDE the arch; costed ONLY when seen, never paired
  REAR_LIGHT_STRIP:   'REAR_LIGHT_STRIP',   // batch 156 — trim/lamp strip across the tailgate between the rear lamps; costed ONLY when seen, never paired
  ...
  // ── VISIBLE-FLAG entries (2) ──────────────────────────────────────────────────────────
  // Fires on visible evidence only — never on surface deformation.
  // FLAG (inspect), NOT a cost entry.

  DISPLACED_WHEEL: 'DISPLACED_WHEEL', // wheel visibly out of position (wrong angle, pushed from arch)
  // AIRBAG: deployed-airbag RECOGNITION marker. Flag-class (never costed) so the model's own
  // airbag £ is gate-stripped by construction (no costed verdict); the SRS injection (assess
  // route) owns the cost via the SRS_AIRBAG price table. Gives the bag an enum home so it is
  // no longer mis-filed under OTHER — and must NEVER get a PANEL_PRICE_TABLE entry of its own.
  AIRBAG: 'AIRBAG',
...
export const PANEL_BEHAVIOUR = Object.freeze({
  ...
  [PANEL.WHEEL_ARCH_MOULDING]: PANEL_CLASS.COST,
  [PANEL.WHEEL_ARCH_LINER]:    PANEL_CLASS.COST,
  [PANEL.REAR_LIGHT_STRIP]:    PANEL_CLASS.COST,
  ...
  [PANEL.AIRBAG]:          PANEL_CLASS.VISIBLE_FLAG, // deployment marker — never costed; SRS injection owns the £
...
export const PANEL_DISPLAY = Object.freeze({
  ...
  [PANEL.WHEEL_ARCH_MOULDING]: 'Wheel arch moulding',
  [PANEL.WHEEL_ARCH_LINER]:    'Wheel arch liner',
  [PANEL.REAR_LIGHT_STRIP]:    'Rear light strip / tailgate garnish',
  ...
  [PANEL.AIRBAG]:          'SRS airbag (deployed)',
