...
export function isBevLot(vd) {
  const fuelType = vd?.fuelType;
  const fuel     = vd?.fuel;
  if (fuelType && DVLA_DEFINITE.test(fuelType)) {
    return DVLA_BEV.test(fuelType);            // DVLA definite → it decides, listing ignored
  }
  return LISTING_BEV.test(fuel || '');         // DVLA absent/unrecognised → listing fallback
}
