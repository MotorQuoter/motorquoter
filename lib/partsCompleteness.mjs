 ...
 * batch 75 §1: also skipped if Call 2 EXPLICITLY listed the panel in namedAsIntact (prose called it
 * sound). KNOWN TRADE (accepted by Vincent 26 Aug): this swaps a TEXT false-positive for a MODEL
 * false-negative — if Call 2 wrongly marks a genuinely damaged, uncosted part as intact, the net will
 * not flag it. EXPLICIT ONLY: silence is not intactness (an unmentioned part still flags), and an
 * absent/empty namedAsIntact reproduces the pre-batch-75 behaviour exactly (fail toward flagging).
 * @returns {Array} flag objects to append to _flaggedParts.
 */
export function reconcileNamedComponents(vdsText, costedPanelIds, flaggedPanelIds, costedText = '', flaggedText = '', namedAsIntact = []) {
  const text = String(vdsText || '');
  if (!text.trim()) return [];
  const costed  = asSet(costedPanelIds);
  const flagged = asSet(flaggedPanelIds);
  const intact  = asSet(Array.isArray(namedAsIntact) ? namedAsIntact : []);
  ...
  for (const c of VDS_COMPONENTS) {
    ...
    if (intact.has(c.panelId)) continue;                             // Call 2 read it as EXPLICITLY sound
...
// Bounded to the fogs: on the gone branch it adds at most FOG_LAMPS_PER_END fog rows (moving parts_sum
// by the genuinely-lost fogs); on the intact branch it is flag-only. `fogSeed` = { oem, used } for the
// vehicle's band, or null (no Brego trade → no band → can't price a seed, so it flags instead).
//
// batch 167 (Vincent, 21 Sep): the REAR end carries ONE fog lamp, not two. UK law requires one rear fog
// lamp; twins are the exception (CK75ONW, an IONIQ 5, has one — batch 166 found the rule seeding two,
// +£105). The front end still carries a pair. "Both … fogs" and "topped up to two" above now read as
// "up to this end's count". ONE owner of the count: this table. If the ledger already holds that many
// (or more — the model wrote two rear fogs), nothing is added.
export const FOG_LAMPS_PER_END = Object.freeze({ front: 2, rear: 1 });
// batch 177 P3 (Vincent, 22 Sep — CHANGES the 31 Jul "bumper gone, foglights gone" rule for ONE case): when the front
// bumper is gone but the front impact is ONE CORNER, only that corner's fog went with it — seed ONE, and ask about the
// second. One corner = damageSpan is not full_width AND fewer than two damaged headlamps were seen (batch 171 P3's
// count); if either says full width, the pair stands. The per-view nearside/offside tags are NOT used (batch 162).
// Only the confirmed "bumper gone" branch changes; the unconfirmed-parent and intact-bumper branches are untouched.
export const FOG_SECOND_ONE_CORNER_LINE = 'Second front fog lamp — confirm it is present and undamaged.';
export function frontImpactIsOneCorner(damageSpan, damagedHeadlampsSeen) {
  return (damageSpan ?? 'full_width') !== 'full_width' && Number(damagedHeadlampsSeen ?? 0) < 2;
}
function fogRuleForEnd(fogParts, bumperGone, endLabel, fogSeed, bumperConfirmed = false, oneCorner = false) {
  const add = [], flags = [];
  const list = Array.isArray(fogParts) ? fogParts : [];
  const n = list.length;
  const target = FOG_LAMPS_PER_END[endLabel] ?? 2;
  const goneTarget = oneCorner && endLabel === 'front' ? 1 : target;   // batch 177 P3
  // The buyer-facing words follow the count: "both front fog lamps sit in it" / "the rear fog lamp sits in it".
  const fogsSit = target === 1 ? `the ${endLabel} fog lamp sits` : `both ${endLabel} fog lamps sit`;
  const fogsLabel = target === 1 ? `${endLabel} fog lamp` : `${endLabel} fog lamps`;
  // batch 71 FIX 4: a SEEDED fog is named for its end ("Rear fog lamp" / "Front fog lamp") so it is
  // distinguishable from a front one in the report — the zone was always carried, only the display
  // name dropped it. A topped-up fog keeps its own real name (list[0]) unchanged.
  const endName = `${endLabel.charAt(0).toUpperCase()}${endLabel.slice(1)} fog lamp`;
  if (bumperGone && n < target && !bumperConfirmed) {
    // §3 (batch 81, Vincent): a part seeded from another part's state may be COSTED only when the parent
    // is CONFIRMED — not when merely disagreed. Under §1 a disagreed bumper survives and _rearBumperOff
    // stays true (or fires from an aperture read on an unconfirmed bumper), which previously seeded two
    // phantom fog costs (DL72FVX: £130 of nothing, the seeded child outliving a gate-stripped parent).
    // The seeded child inherits the parent's confidence: an unconfirmed parent yields a FLAG, never a
    // cost — surfaced for inspection, priced by the buyer (batch 82) if it matters.
    flags.push({
      panelId: PANEL.FOG_LAMP, partName: target === 1 ? `${endLabel} fog lamp` : `${endLabel} fog lamp(s)`, zone: endLabel, weight: 'low',
      reason: `The ${endLabel} bumper was read as displaced but its condition could not be confirmed across the photos. If it is genuinely off, ${fogsSit} in it — surfaced for inspection, NOT costed. Confirm on the WhatsApp inspection and price it yourself if it applies.`,
      _completenessNet: true, _fogCheck: true, _fogUnconfirmedParent: true,
    });
    return { add, flags };
  }
  if (bumperGone) {
    // batch 177 P3: a one-corner front asks about the second fog instead of costing it — whether or not a fog is seeded.
    const secondCheck = goneTarget < target && n < target
      ? { panelId: PANEL.FOG_LAMP, partName: 'Second front fog lamp', zone: endLabel, weight: 'medium',
          reason: FOG_SECOND_ONE_CORNER_LINE, _completenessNet: true, _fogCheck: true, _fogSecondOneCorner: true }
      : null;
    if (n >= goneTarget) { if (secondCheck) flags.push(secondCheck); return { add, flags }; }   // already this end's count costed
    ...
    const template = list[0] || (fogSeed
      ? { panelId: PANEL.FOG_LAMP, name: endName, action: 'replace', oem: null, used: fogSeed.used ?? fogSeed.oem ?? null }
      : null);
    if (!template || (template.used == null && template.oem == null)) {
      ...
      flags.push({
        panelId: PANEL.FOG_LAMP, partName: fogsLabel, zone: endLabel, weight: 'medium',
        reason: `${endLabel} bumper is gone — ${fogsSit} in it and should be costed; no price band was available, so confirm and cost on inspection.`,
        _completenessNet: true, _fogCheck: true,
      });
      return { add, flags };
    }
    for (let i = 0; i < goneTarget - n; i++) {
      // gatedParts rows are keyed on `name`; carry zone too so a later front/rear read still works.
      add.push({ ...template, panelId: PANEL.FOG_LAMP, name: template.name || endName, action: 'replace', zone: endLabel, _fogPaired: true });
    }
    if (secondCheck) flags.push(secondCheck);
    return { add, flags };
  }
  // Intact bumper, fewer fogs than this end carries → "check the second". batch 167: the rear carries one,
  // so one costed rear fog is the full count and there is no second to check (front: unchanged, n === 1).
  if (n >= 1 && n < target) {
    flags.push({
 ...
 */
export function applyFogBumperRule({ costedParts, frontBumperGone, rearBumperGone, frontBumperConfirmed = false, rearBumperConfirmed = false, fogSeed = null, frontOneCorner = false } = {}) {
  ...
  const f = fogRuleForEnd(frontFogs, !!frontBumperGone, 'front', fogSeed, !!frontBumperConfirmed, !!frontOneCorner);   // batch 177 P3
  const r = fogRuleForEnd(rearFogs,  !!rearBumperGone,  'rear', fogSeed, !!rearBumperConfirmed);
...
export function completenessFlagsFor(assessment, gatedParts, buildBuyerFlags, namedAsIntact = []) {
  const a = assessment || {};
  ...
  return reconcileNamedComponents(a['Visible Damage Summary'], costedPanelIds, flaggedPanelIds, costedText, flaggedText, namedAsIntact);
}
