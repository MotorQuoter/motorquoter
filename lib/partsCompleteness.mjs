...
function fogRuleForEnd(fogParts, bumperGone, endLabel, fogSeed, bumperConfirmed = false) {
  const add = [], flags = [];
  ...
  if (bumperGone) {
    if (n >= target) return { add, flags };                  // already this end's count costed
 ...
 */
export function applyFogBumperRule({ costedParts, frontBumperGone, rearBumperGone, frontBumperConfirmed = false, rearBumperConfirmed = false, fogSeed = null } = {}) {
  ...
  const f = fogRuleForEnd(frontFogs, !!frontBumperGone, 'front', fogSeed, !!frontBumperConfirmed);
