...
import {
  CO2_BANDS, VRT_MINIMUM, NOX_TIERS, NOX_NO_EVIDENCE_CHARGE, EURO_NOX_ESTIMATE,
...
function computeNox({ fuel, euroClass, noxOverride }) {
  ...
  // ⚠️ batch 53 gap 2: this uses Revenue's NO-EVIDENCE DEFAULT as a CEILING on the Euro-standard
  // estimate. Revenue document €4,850/€600 as the default WHEN evidence is absent, not as a maximum on
  // a computed charge — so clamping an estimate to it may UNDERSTATE tax (a Euro-3 diesel estimate of
  // 500 mg/km computes to ~€11,300, currently clamped to €4,850). Behaviour retained pending the
  // Revenue-calculator TASK-0; do NOT remove the clamp without that answer (removing it RAISES the quote).
  const cap = cls === 'diesel' ? NOX_NO_EVIDENCE_CHARGE.diesel : NOX_NO_EVIDENCE_CHARGE.other;
...
export const CAT_C_AGE_MONTHS = 30 * 12; // >30 years at registration → Category C (flat €200)
...
export function classifyNewMeansOfTransport({ ageMonths, odometerKm } = {}) {
  ...
  // A >30-year-old car cannot be a "new means of transport" — the low-km limb must not flag a classic.
  const isClassic = ageKnown && ageMonths > CAT_C_AGE_MONTHS;
  const ageNew = ageKnown && ageMonths <= NEW_MEANS_AGE_MONTHS;
  const kmNew  = kmKnown  && odometerKm <= NEW_MEANS_KM;
  const isNew  = !isClassic && (ageNew || kmNew);
...
// Import scope gate: the product covers Category A passenger cars (type approval M1) ONLY.
// Commercials (N1/N2/N3), buses (M2/M3), tractors (T1–T5), and absent/unrecognised are REFUSED
// before payment (a van's VRT is a different regime, and seats/laden-mass aren't in the data we
// hold). Returns a refusal reason string, or null when in scope. Never assume M1 on missing data.
export function importScopeRefusal(typeApproval) {
  const ta = String(typeApproval || '').toUpperCase().trim();
  if (ta === 'M1') return null;
  if (/^N[123]$/.test(ta)) return 'This looks like a commercial vehicle (van or truck). Our import cost check covers passenger cars only — commercial VRT is a different regime. Check Revenue for commercial rates.';
  if (/^M[23]$/.test(ta)) return 'This looks like a bus or coach. Our import cost check covers passenger cars only. Check Revenue for these rates.';
  if (/^T[1-5]$/.test(ta)) return 'This looks like an agricultural tractor. Our import cost check covers passenger cars only.';
  return 'We couldn’t confirm this is a passenger car (type approval M1) from the DVLA record, so we can’t give an import cost for it.';
}
...
export function estimateImportCost({
  ...
  // ── Category C by age: a car more than 30 years old AT REGISTRATION pays a flat €200 VRT
  // (Category C), NOT the CO2/NOx computation. Age is measured at the time of registration in the
  // State, not today — a car near the line may cross it before it is presented, so flag "near".
  const ageC = Number.isFinite(ageMonths) && ageMonths >= 0;
  const categoryC = ageC && ageMonths > CAT_C_AGE_MONTHS;
  const categoryCNear = ageC && !categoryC && ageMonths >= CAT_C_AGE_MONTHS - 6; // within 6 months of 30 years

  // ── VRT ──
  let vrt = null;
  if (categoryC) {
    vrt = {
      total: 200, categoryC: true,
      co2Charge: null, noxLevy: 0, noxBasis: 'Category C (over 30 years) — flat €200; CO2 and NOx do not apply.',
      floorApplied: false, band: null,
    };
    notes.push('Over 30 years old at registration — Category C flat €200 VRT. CO2 and NOx do not apply.');
  } else {
  ...
  if (co2Part) {
    ...
    if (isEV) notes.push('Electric vehicle — lowest CO2 band applied and NOx is €0. A VRT relief of up to €5,000 may further reduce the amount, but ONLY for EVs REGISTERED in Ireland before 31 December 2026 — registered, not bought or imported (buy in December and register in January and the relief is lost). The relief is up to €5,000 at an OMSP of €40,000 or less, tapers between €40,000 and €50,000, and does not apply above €50,000. We do not compute it — check Revenue.');
  } else {
  ...
  if (categoryCNear) notes.push('This car is close to 30 years old. VRT is measured at the date you register it in Ireland — if it passes 30 years by then it becomes a flat €200 (Category C). Check the exact date.');
  ...
  return { supported: true, vrt, vat, customsDutyFlag, grandTotal, basis, notes, newMeansOfTransport: nmt, categoryC, categoryCNear };
}
