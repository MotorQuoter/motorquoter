...
// Parse a Euro standard (1–6) from a discrete class ("Euro 6"), a number, or a derivative
// string that embeds it ("… 2.0 TDI Euro 6 …"). Returns an integer 1–6 or null.
export function parseEuroClass(euroClass) {
  ...
  const m = String(euroClass).match(/euro\s*([1-6])/i);
...
export function estimateImportCost({
  ...
  if (prov === 'NI') {
    ...
    customsDutyFlag = {
      ...
      note: `Customs duty is ${Math.round(CUSTOMS_DUTY_RATE * 100)}% of the customs value UNLESS the car qualifies as UK preferential-origin under the TCA (then €0). Origin cannot be determined automatically — treat as an open item. Not included in the total below.`,
    };
