...
// Retention sweep. A stored report can only ever be served inside the 10-minute window, so a row
// older than the TTL is unreadable dead weight holding the full report payload — the wrong posture
// for an ICO-registered B2C product. Delete expired rows opportunistically on each write (the paid
// path is the ONLY thing that grows the table, so sweeping on write keeps it self-limiting with no
// new cron surface). Uses the existing created_at index. Best-effort: never throws, never affects the
// write's own success.
export async function sweepExpiredStoredReports(supabase, nowMs = Date.now()) {
  if (!supabase) return;
  try {
    const cutoff = new Date(nowMs - STORED_REPORT_TTL_MS).toISOString();
    const { error } = await supabase.from('paid_reports').delete().lt('created_at', cutoff);
    if (error) console.error(`[STORED REPORT] sweep failed (non-fatal): ${error.code || ''} ${error.message || error}`);
  } catch (err) {
    console.error('[STORED REPORT] sweep exception (non-fatal):', err?.message || err);
  }
}
...
export async function writeStoredReport(supabase, { sessionId, vrm, checks, market, payload }) {
  ...
  try {
    ...
    // Opportunistic retention sweep — self-limiting, indexed, best-effort; independent of this return.
    await sweepExpiredStoredReports(supabase);
    return true;
  } catch (err) {
