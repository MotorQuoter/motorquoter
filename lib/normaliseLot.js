...
function parseCopart(raw) {
  ...
  if (!vatOnSale && /VAT\s+to\s+be\s+added/i.test(raw)) {
  ...
  return {
    ...
    cleanedDescription: cleanCopartDescription(raw),
  };
}
...
//
// bodyStyle is intentionally NOT in the explicit merge list — same as the original
// enrichedVd construction (parsed.bodyStyle was extracted but not merged; the field
// comes from ...rawVd spread = vd.bodyStyle from session, typically undefined from
// the current form). Fix bodyStyle in a separate commit after validation.
export function normaliseLot(rawVd) {
  const source = rawVd.auctionSource || 'copart';
  const parsed = source === 'copart'  ? parseCopart(rawVd.damageDescription)
