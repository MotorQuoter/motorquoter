...
// Source: Vincent trade-knowledge price grid, Jun 2026.
// 23 cost panels — HEADLAMP and GRILLE excluded (both keep their existing code-owned logic).
...
export const PANEL_PRICE_TABLE = Object.freeze({
  ...
  // batch 75 §3 — Vincent's anchor UPPER_EXEC £120 oem / £60 used (£60 market-verified across two
  // listings); FRONT_BUMPER band progression applied around it, £5-rounded, no S/H exceeds its OEM.
  // Cost ONLY when a moulding is actually seen — NEVER paired/seeded (unlike fogs, it lives in nothing).
  WHEEL_ARCH_MOULDING: {
    [BAND_KEYS.ECONOMY]:      { oem:  60, used:  30 },
    [BAND_KEYS.MID_RANGE]:    { oem:  80, used:  40 },
    [BAND_KEYS.EXECUTIVE]:    { oem: 100, used:  50 },
    [BAND_KEYS.UPPER_EXEC]:   { oem: 120, used:  60 },
    [BAND_KEYS.PRESTIGE]:     { oem: 150, used:  75 },
    [BAND_KEYS.LUXURY]:       { oem: 180, used:  90 },
    [BAND_KEYS.SUPER_LUX]:    { oem: 240, used: 120 },
  },
  // batch 156 T1 — TRIM ITEMS. Vincent gave ONE figure each, taken as the UPPER_EXEC OEM anchor:
  // wheel arch liner £75, rear light strip £100. The moulding precedent (batch 75 §3) then sets the rest:
  // used = 50% of OEM, band progression ×0.5 / ×2⁄3 / ×5⁄6 / ×1 / ×1.25 / ×1.5 / ×2, £5-rounded (.5 up).
  // Labour: NEITHER is in BODY_PANEL_LABOUR (lib/labour.mjs) — trim is "minutes, absorbed"
  // (LABOUR_SPEC_v1 §411), the same as the moulding. Cost ONLY when seen; never paired, never seeded.
  WHEEL_ARCH_LINER: {
    [BAND_KEYS.ECONOMY]:      { oem:  40, used:  20 },
    [BAND_KEYS.MID_RANGE]:    { oem:  50, used:  25 },
    [BAND_KEYS.EXECUTIVE]:    { oem:  65, used:  35 },
    [BAND_KEYS.UPPER_EXEC]:   { oem:  75, used:  40 },
    [BAND_KEYS.PRESTIGE]:     { oem:  95, used:  50 },
    [BAND_KEYS.LUXURY]:       { oem: 115, used:  60 },
    [BAND_KEYS.SUPER_LUX]:    { oem: 150, used:  75 },
  },
  REAR_LIGHT_STRIP: {
    [BAND_KEYS.ECONOMY]:      { oem:  50, used:  25 },
    [BAND_KEYS.MID_RANGE]:    { oem:  65, used:  35 },
    [BAND_KEYS.EXECUTIVE]:    { oem:  85, used:  45 },
    [BAND_KEYS.UPPER_EXEC]:   { oem: 100, used:  50 },
    [BAND_KEYS.PRESTIGE]:     { oem: 125, used:  65 },
    [BAND_KEYS.LUXURY]:       { oem: 150, used:  75 },
    [BAND_KEYS.SUPER_LUX]:    { oem: 200, used: 100 },
  },
  BONNET: {
  ...
  // ── SRS airbag deployment — injection-only pseudo-panels (NOT a PANEL enum member) ──────────
  // Code-injected on PHOTO-CONFIRMED deployment (assess route), tier by deployment extent.
  // Keyed SRS_AIRBAG_T1/T2/T3 and looked up by the assess route's dedicated SRS deployment
  // vision read (runSrsDeploymentRead → tier); the injected row carries the literal 'SRS_AIRBAG'
  // sentinel panelId. These are never read by the vision pass and never appear in the panel vocabulary.
  //   T1 single zone (driver only) · T2 front pair · T3 multi-zone (fronts + curtains/side).
  // ⛔ NOT USED FOR THE REPAIR TOTAL SINCE BATCH 130. Vincent ruled (14 Sep, spec §10) ONE FLAT £500
  // FLOOR for any confirmed deployment, whatever the tier: the injected row is the flat floor
  // (SRS_FLOOR_GBP, lib/labour.mjs) printed "from £500", and it carries _srsFloor, NOT _srsTier.
  // Consequences, verified at HEAD:
  //   • NOTHING outside this file reads SRS_AIRBAG_T1/T2/T3 — the tier bands below set no buyer-facing
  //     money at all. The tier is still read and logged by the assess route, but it sets no figure.
  //   • the SRS fitting rider (SRS_FITTING / srsFitting, lib/labour.mjs) is dead for the same reason:
  //     it fires only on _srsTier, which no row carries. The replays log `srs=£0` on every lot.
  // So `used` and `oem` below are dormant reference figures, not a price anything charges. They are
  // kept, not deleted, because Vincent rules on removal.
  // (Batch 125's "supply-only kit + named fitting rider" reading was superseded by batch 130 before it
  // ever shipped; the branch carrying it was deleted in batch 149 rather than merged.)
  SRS_AIRBAG_T1: {
    [BAND_KEYS.ECONOMY]:      { oem: 410, used: 225 },
    [BAND_KEYS.MID_RANGE]:    { oem: 565, used: 310 },
    [BAND_KEYS.EXECUTIVE]:    { oem: 750, used: 410 },
    [BAND_KEYS.UPPER_EXEC]:   { oem: 980, used: 540 },
    [BAND_KEYS.PRESTIGE]:     { oem: 1280, used: 705 },
    [BAND_KEYS.LUXURY]:       { oem: 1635, used: 900 },
    [BAND_KEYS.SUPER_LUX]:    { oem: 2315, used: 1275 },
  },
  SRS_AIRBAG_T2: {
    [BAND_KEYS.ECONOMY]:      { oem: 680, used: 375 },
    [BAND_KEYS.MID_RANGE]:    { oem: 940, used: 515 },
    [BAND_KEYS.EXECUTIVE]:    { oem: 1245, used: 685 },
    [BAND_KEYS.UPPER_EXEC]:   { oem: 1635, used: 900 },
    [BAND_KEYS.PRESTIGE]:     { oem: 2135, used: 1175 },
    [BAND_KEYS.LUXURY]:       { oem: 2725, used: 1500 },
    [BAND_KEYS.SUPER_LUX]:    { oem: 3860, used: 2125 },
  },
  SRS_AIRBAG_T3: {
    [BAND_KEYS.ECONOMY]:      { oem: 1090, used: 600 },
    [BAND_KEYS.MID_RANGE]:    { oem: 1500, used: 825 },
    [BAND_KEYS.EXECUTIVE]:    { oem: 1995, used: 1095 },
    [BAND_KEYS.UPPER_EXEC]:   { oem: 2615, used: 1440 },
    [BAND_KEYS.PRESTIGE]:     { oem: 3420, used: 1880 },
    [BAND_KEYS.LUXURY]:       { oem: 4365, used: 2400 },
    [BAND_KEYS.SUPER_LUX]:    { oem: 6175, used: 3395 },
  },
