...
//
// COMPLEMENT (ownership split of record — see lib/sideScrub.mjs header): this layer owns
// PROSE-WIDE compound side-terms with SUBSTITUTION semantics. The residual it does NOT own —
// bare "left"/"right" and the abbreviations "n/s"/"o/s", removed on PART-NAME CELLS ONLY —
// belongs to scrubSideWords (lib/sideScrub.mjs). One owner per token class; no overlap. Bare
// left/right are deliberately absent HERE: prose-wide removal is unsafe ("left of the bay").
//
...
export function sanitizeSideTerms(text) {
  ...
  // Step 2b (batch 141 item 6a): drop a DANGLING substituted side phrase.
  // HV25ODX Key Cost Drivers shipped "Windscreen: cracked through on the opposite-side -
  // full replacement." The ban WORKED — the model's absolute side label was caught at Step 2 —
  // but substitution is not removal: it left a bare locative that names no part, tells the buyer
  // nothing, and still asserts a side. Whenever "on/at/to the damaged-side|opposite-side" is
  // TERMINAL (no noun after it), the phrase carries no information, so it goes. Used as an
  // ADJECTIVE it is kept untouched — "the opposite-side front wing", "the damaged-side front
  // wheel" are exactly the forms the prompt asks for, and a noun follows.
  s = s.replace(/\s+(?:on|at|to|along)\s+the\s+(?:damaged|opposite)-side(?=\s*(?:[.,;:)\]]|\u2014|\s-\s|$))/gi, '');
