...
// row: image_paths → null (nothing can try to fetch what is gone), images → null (legacy base64), and
// images_purged_at → now() as the POSITIVE expiry fact the honest 410 hangs on. No DELETE — so no
...
async function purgeAssessment(supabase, row) {
  ...
  const { error: upErr } = await supabase
    ...
    .update({ image_paths: null, images: null, images_purged_at: new Date().toISOString() })
    .eq('id', row.id);
