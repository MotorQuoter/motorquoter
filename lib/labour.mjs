...
// ── batch 129 — THE SRS RULING ON THE PAGE (Vincent, 14 Sep: "Airbags out, please check, replacement from £500
// depending on number and location." · on the wording: "must be checked reads better.") ────────────────────────
//   1. The engine does NOT count bags — deployment is flagged, the count is never claimed.
//   2. The band table is unchanged; its figure is presented as an APPROXIMATE FROM figure, never a computed cost.
//      Vincent, 14 Sep (batch 129): the from-figure is KIT + FITTING (band `used` + the tier's rider) — e.g. the
//      lowest possible, Economy T1 £225 + £300 = £525, is his "from £500".
//   3. The collateral is NAMED, not costed.
// ONE owner of the buyer-facing sentence (the inspection flag). Latin-1 only: the PDF's flag renderer drops en/em
// dashes, so the dash is a hyphen. No figure here is invented: kit and fitting come from the band table and
// SRS_FITTING. Money is untouched — the kit row and the fitting row stay in the total exactly as before.
...
export function srsDeploymentNote({ kit, fitting } = {}) {
  const from = (Number(kit) || 0) + (Number(fitting) || 0);
  return `Airbags deployed - replacement from £${from.toLocaleString('en-GB')} (kit and fitting), depending on the number and location of the bags. ${SRS_COLLATERAL} This must be checked before bidding.`;
}
// A ledger row whose figure is a FROM figure, not a computed cost: the £500 jig floor (batch 106), and the two SRS
// rows (batch 129 — the kit row carries _srsTier, the fitting row _srsFitting). Display only.
export function isFromFigureRow(row) { return !!(row?._structFloor || row?._srsTier || row?._srsFitting); }
