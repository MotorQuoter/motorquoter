...
//   SECOND-HAND : bolt-ons REPLACED are sourced colour-matched, ADDITIVE ~£170 each, NO flatten (unpainted);
//                 welded panels have NO s/h price → carry their welded figure, painted. Part cost + structural
//                 allowance sit OUTSIDE both columns and are added once. −15/+25 applies to each column's panel work.
// batch 127 (spec open item 4 — Vincent, 14 Sep: "fix it, then show it"): a REPAIRED bolt-on gets no new panel,
// so there is no £170 to fit. It keeps its painted repair figure instead (the repair still happens on the
// second-hand route) and joins the PAINTED set — welded + repaired bolt-ons, flattened together per zone.
// Only a REPLACED bolt-on is sourced finished at £170. DISPLAY ONLY — nothing in the money reads this column.
...
export function assembleColumns(panels = []) {
  const meta = panels.map((p) => {
    const welded = !!weldedClass(p.panelId);
    return { zone: p.zone, welded, repaired: !welded && String(p.action || '').toLowerCase() === 'repair', labour: panelLabour(p) };
  });
  const newPaintedPW = flattenPanelWork(meta.map((p) => ({ zone: p.zone, labour: p.labour })));            // all painted, flattened together
  const painted = meta.filter((p) => p.welded || p.repaired);                                               // painted on the car on either route
  const sourced = meta.filter((p) => !p.welded && !p.repaired);                                             // replaced bolt-ons: sourced finished
  const secondHandPW = flattenPanelWork(painted.map((p) => ({ zone: p.zone, labour: p.labour })))          // painted set flattens among itself
                     + sourced.length * SOURCED_FINISHED_FIT;                                               // sourced bolt-ons additive, no flatten
  return {
...
// ── batch 127 — THE RANGE ON THE PAGE (spec §6 + open items 3/4, wording approved by Vincent 14 Sep) ──────
// ONE owner of the three buyer-facing labour sentences, read by the screen and the PDF through applyEdits
// (edited.labourDisplay), so the two surfaces cannot word it differently. Hyphen, never an en dash: the PDF is
// Latin-1 only. DISPLAY ONLY — the figures come from the stored/recomputed columns; nothing here is money.
// ⛔ NOT ranged (§6): the £500 structural floor, the SRS fitting rider, the structural allowance.
// batch 161 E4 (Vincent, 19 Sep) — the old sentence told the buyer to "remove the line and add their
// figure", which is two steps and, since batch 158 A2 took the controls off this row, was pointing at a
// button that did not exist. It now names the one control that does the job.
export const LABOUR_RANGE_ADDENDUM = 'Labour & paint is an estimate, shown as a range. The repair total, margins and bid ceilings all use the top of that range. If your repairer quotes a different figure, press Change on this line and enter it.';
const _gbp = (n) => `£${Number(n).toLocaleString('en-GB')}`;
const _isFig = (v) => v !== null && v !== undefined && v !== '' && Number.isFinite(Number(v));
export function labourDisplayLines(columns) {
  const np = columns?.newPainted;
  if (!np || !_isFig(np.low) || !_isFig(np.high)) return null;
  const sh = columns?.secondHand;
  return {
    range: `Estimate ${_gbp(np.low)} - ${_gbp(np.high)} · the total uses the top`,
    secondHand: sh && _isFig(sh.low) && _isFig(sh.high)
      ? `With second-hand colour-matched panels: ${_gbp(sh.low)} - ${_gbp(sh.high)} · for comparison, not in the total`
      : null,
    addendum: LABOUR_RANGE_ADDENDUM,
  };
}
...
// ── batch 130 — Q4 DOES NOT PROMOTE ON A LOSING VOTE (Vincent, 14 Sep: "One damaged read against three is not a
// genuine disagreement. Flag it, do not cost it.") ─────────────────────────────────────────────────────────────
// HMZ8034: _pvVotes.REAR_QUARTER {views 4, damaged 1, clean 2} was flagged honestly, then costed at the band default
// anyway; Vincent confirmed by eye that the quarter is SOUND. The boundary (Cowork's reading, adopted as stated):
// promote only when damaged >= clean — a tie is still a genuine disagreement and still promotes. Where damaged <
// clean: flag only, no cost. NARROWS the 31 Aug fail-safe ("a cost he can see and delete is safe"); does not repeal it.
// Build choices, stated: every pvVotes entry keyed REAR_QUARTER or REAR_QUARTER#n (a G-split instance) is read; the
// quarter is declined only when EVERY such entry is a losing vote; no vote entry at all → nothing to weigh → promotes
// as before. The flag itself is never removed — the panel stays in the flags and the damage breakdown at £0.
export function isLosingQuarterVote(pvVotes) {
  const entries = Object.entries(pvVotes || {})
    .filter(([k, v]) => (k === 'REAR_QUARTER' || k.startsWith('REAR_QUARTER#')) && v && typeof v === 'object')
    .map(([, v]) => v);
  if (!entries.length) return false;
  return entries.every((v) => (Number(v.damaged) || 0) < (Number(v.clean) || 0));
}

// ── batch 132 — THE DECLINED QUARTER REACHES THE BUYER (Vincent, 15 Sep: "It should be in the check list.") ───────
// A quarter Q4 declines has no model row in _preGateParts, so the 4f filter (buildBuyerFlags) hid its flag and the
// checklist never saw it — the buyer met it only as a £0 Related damage card. ONE OWNER of the mark: the decline below
// stamps _q4Declined on the quarter's vote-derived flags (the disagree / single-MINOR flags whose votes lost). The 4f
// filter and the checklist seed READ that mark; neither re-derives the vote test. Both buyer strings live here.
//   Flag reason — says the photos disagree, the quarter is not costed, and it must be checked. Latin-1 (a hyphen, not a
//   dash): the PDF prints flag reasons without str(), so an em dash there would be dropped.
//   Checklist item — Vincent's/Cowork's wording verbatim; the checklist path maps the em dash to a hyphen in the PDF.
export const Q4_DECLINED_REASON = 'The photos disagree on this panel - it is not costed and must be checked before bidding.';
export const Q4_DECLINED_CHECKLIST_ITEM = 'Show rear quarter close-up — the photos disagree on this panel; it is not costed and must be checked before bidding.';

export function promoteFlaggedQuarter({ gatedParts, flaggedParts = [], costedIds, sevByPanel, zoneByPanel, entry, name, pvVotes = null } = {}) {
  if (!entry) return null;
  for (const f of flaggedParts || []) {
    if (f?.panelId !== 'REAR_QUARTER' || costedIds.has('REAR_QUARTER')) continue;
    if (isLosingQuarterVote(pvVotes)) {
      const votes = Object.fromEntries(Object.entries(pvVotes).filter(([k]) => k === 'REAR_QUARTER' || k.startsWith('REAR_QUARTER#')));
      // batch 132: mark the quarter's vote-derived flags (the ones whose votes lost) — the flag objects are shared with
      // _flaggedParts, the damage cards and the checklist seed, so the mark and the reason reach every one of them.
      let marked = 0;
      for (const q of flaggedParts) {
        if (q?.panelId !== 'REAR_QUARTER' || !(q._amalgDisagree || q._amalgSingleMinor)) continue;
        q._q4Declined = true;
        q.reason = Q4_DECLINED_REASON;
        marked++;
      }
      return { row: null, declined: votes, marked };   // batch 130: flag only — no row, no grade, no money
    }
...
// SRS airbag fitting rider (spec §10) — Vincent's tiered figures (300/600/1000). ⛔ DO NOT interpolate or scale.
// batch 130: SETS NO FIGURE. The SRS money is the flat £500 floor below; no row carries _srsTier any more, so the
// labour reconcile passes no tier and computeLabour's rider is £0 on every lot. Kept, not deleted — Vincent rules
// on removal.
export const SRS_FITTING = Object.freeze({ T1: 300, T2: 600, T3: 1000 });
export function srsFitting(tier) { return SRS_FITTING[tier] || 0; }

// ── batch 130 — SRS IS ONE FLAT £500 FLOOR (Vincent, 14 Sep: "Airbags out, must be checked, replacement from £500
// depending on number and location." · "Yes £500 is counted in the repair with the note.") ─────────────────────
// SUPERSEDES batch 129's tier-derived from-figure (band kit + tier rider), which rendered "from £2,880" on HMZ8034:
// a ceiling wearing a floor's label. Same shape as the jig floor (spec §5):
//   1. ONE figure, £500, for ANY deployment, whatever the tier. No band, no tier. ⛔ Do not interpolate, scale or
//      band it.
//   2. It goes IN the repair total as ONE row — it replaces both the tiered kit row and the fitting rider.
//   3. Number and location live in the WORDS, never in the figure. "Must be checked". The collateral is NAMED, not
//      costed.
// ONE owner of the buyer-facing sentence (the inspection flag). Latin-1 only: the PDF's flag renderer drops en/em
// dashes, so the dash is a hyphen.
export const SRS_FLOOR_GBP = 500;
export const SRS_COLLATERAL = 'The headlining, seat covers, dashboard, door cards and seatbelt pretensioners may also need replacing and are not costed.';
export function srsDeploymentNote() {
  return `Airbags deployed - replacement from £${SRS_FLOOR_GBP} (kit and fitting), depending on the number and location of the bags. ${SRS_COLLATERAL} This must be checked before bidding.`;
}
// A ledger row whose figure is a FLOOR ("from £X"), not a computed cost: the £500 jig floor (batch 106) and the £500
// SRS floor (batch 130). Display only. batch 129's _srsTier / _srsFitting markers no longer print "from" — 129 never
// shipped, so a stored report renders its old SRS rows exactly as main does.
export function isFromFigureRow(row) { return !!(row?._structFloor || row?._srsFloor); }

// ── batch 131 — ONE owner of the Parts Breakdown cells, screen AND PDF (Vincent, 15 Sep: "£500 is the bottom of a
// range.") ─────────────────────────────────────────────────────────────────────────────────────────────────────
// Batch 130's SRS floor row is action 'replace', so the old per-renderer costCells put its £500 in the S/H column and
// printed it BARE — the "from" test only looked at the Repair cost column. A floor row now always takes the Repair
// cost column (the jig floor's column, action 'inspect') and carries `from: true`; the renderer prints "from £X"
// there and nowhere else. Every other row is placed exactly as before. Position only — no figure is recomputed.
//   replace → OEM + S/H · repair → OEM + Repair cost · labour/other → Repair cost · FLOOR (isFromFigureRow) → Repair cost, "from"
export function partsTableCells(p) {
  if (isFromFigureRow(p)) return { oem: null, sh: null, repair: p.used ?? p.oem ?? null, from: true };
  const act = String(p?.action || '').toLowerCase();
  if (act === 'replace') return { oem: p.oem ?? null, sh: p.used ?? null, repair: null, from: false };
  if (act === 'repair')  return { oem: p.oem ?? null, sh: null, repair: p.used ?? p.oem ?? null, from: false };
  return { oem: null, sh: null, repair: p.used ?? p.oem ?? null, from: false };
}
