...
// ⛔ NOT the welded/bonded three (quarter / sill / roof): the spec keys their all-in figure on REPAIR vs
// REPLACE (not on grade) and never states whether a welded panel carries a part cost — a money decision the
// spec does not answer, so they are left exactly as they are (batch 116 report).
...
// ⛔ Rows with no ledger grade are left alone (the grade is the owner; no grade, no ruling).
...
export function applyGradeOwnsAction(gatedParts, sevByPanel) {
  ...
  for (let i = 0; i < (gatedParts || []).length; i++) {
    ...
    if (!r?.panelId || r._zeroRule || /labour|paint|prep/i.test(r.name || '')) continue;
    if (!isBodyPanel(r.panelId) || weldedClass(r.panelId)) continue;
    ...
    if (grade === 'MINOR' || grade === 'MODERATE') {
      ...
      changes.push({ panelId: r.panelId, grade, from: `${r.action} £${r.used ?? r.oem ?? 0}`, to: 'repair £0 part (panel work carries it)' });
