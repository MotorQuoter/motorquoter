 ...
 */
export async function withOneAutoCache(callType, reg, paramsOrFetch, maybeFetch, maybeMeta) {
  ...
  // the legacy shape.
  // batch 136 task D1: an optional META object (the slot after fetchFn) is filled with how the result was obtained —
  // meta.cacheKey and meta.cache = 'replay' | 'hit' | 'miss' | 'stale-served' | 'miss-null' — so a caller can record every
  // One Auto call on the session. A cache HIT carries no HTTP status; this is what says so.
  const legacy  = typeof paramsOrFetch === 'function';
  const params  = legacy ? {} : (paramsOrFetch || {});
  const fetchFn = legacy ? paramsOrFetch : maybeFetch;
  const meta    = (legacy ? maybeFetch : maybeMeta) || {};
  const normReg = reg.replace(/\s+/g, '').toUpperCase();
  if (_replayProvider) { meta.cache = 'replay'; return _replayProvider(callType, normReg); }   // replay-only; null in prod
  ...
  meta.cache = 'miss-null';   // batch 136 D1: live call gave nothing and no stale copy exists — never cached
  return null;
}
