...
export function seedChecklistFromFlags(checklistText, buyerFlags, { lampTier2Fired = false } = {}) {
  ...
  for (const flag of buyerFlags || []) {
    ...
    } else if (flag._bumperOffLimit === true && flag.reason) {
      // batch 189 P3 (Vincent, 24 Sep; the batch 165 ruling): the §4 bumper-off note is on a CHARGED panel, so the checklist
      // says what the flag says. It used to fall through to the default "condition could not be confirmed from the listing
      // photographs", which contradicted the flag and the charged wing line.
      seedItem = flag.reason;
...
// batch 187 P1 (Vincent, 24 Sep) — A FULL STOP INSIDE A NUMBER OR AN ABBREVIATION IS NOT A SENTENCE END. The old split
// ended a sentence at every . ! ? — "Santa Fe 2.2 CRDi" became "2." + "2 CRDi", and the binder's re-join printed
// "2. 2". A sentence now ends at a run of . ! ? (plus any closing quote/bracket) only when it is followed by the end of
// the text, a line break, or whitespace and then an uppercase letter or an opening quote/bracket. So "2.2", "1.0",
// "ID.4", "ID. 4", "c.41,700" and "c. 41,700" never split; "e.g. The" does split (an uppercase letter follows).
// _sentenceSpans keeps each sentence's offsets so a caller can cut text out of the ORIGINAL string (bindClaimClasses)
// instead of re-joining pieces. Each span's text is trimmed; empty spans are dropped.
const _SENT_END_RX = /[.!?]+[)"'”’\]]*(?=$|\s*\n|\s+[A-Z"'(“‘[])/g;
function _sentenceSpans(s) {
  const src = String(s ?? '');
  const spans = [];
  const push = (a, b) => {
    ...
    const t = raw.trim();
    if (t) spans.push({ text: t, start: a + lead, end: a + lead + t.length });
  };
  let from = 0;
  for (const m of src.matchAll(_SENT_END_RX)) { push(from, m.index + m[0].length); from = m.index + m[0].length; }
  push(from, src.length);
  return spans;
}
function _splitSentences(s) {
  return _sentenceSpans(s).map((x) => x.text);
}
export { _splitSentences as splitSentencesForTest };   // batch 187: validate-batch187 pins the split rule
...
function _sentenceContradicts(sentence, ctx, mode) {
  ...
  // batch 191 P1 (Vincent, 25 Sep): the word must be the named PART's action, not the name of the whole job. "The panel
  // repair itself (bumper, wheel, tyre, arch trim, refinish) is light-to-moderate" names the job and lists its content;
  // it says nothing about the wheel's action (KT73YAJ lost its band-position sentence to this). See _actionBoundToPart.
  for (const [name, action] of ctx.partActions) {
    ...
    for (const cl of s.split(/\s+—\s+|;\s+|,\s+plus\s+|,\s+and\s+/)) {
      ...
      if (saysRepair && !saysReplace && action === 'replace' && _actionBoundToPart(cl, name, 'repair')) return { class: 'action', reason: `says repair; "${name}" row action is replace` };
      if (saysReplace && !saysRepair && action === 'repair' && _actionBoundToPart(cl, name, 'replace'))  return { class: 'action', reason: `says replace; "${name}" row action is repair` };
    }
  }
...
// batch 191 P1 (Vincent, 25 Sep) — IS THE ACTION WORD THE PART'S ACTION, OR THE NAME OF THE WHOLE JOB?
// Each "repair"/"replace" word in the clause is read on its own:
//   • a verb or participle ("repaired", "repairing", "replace", "replaced", "replacing") is an action → BOUND
//     ("the bumper is repaired", "repair the bonnet", "a repaired wing");
//   • a noun ("repair", "repairs", "replacement(s)") is the NAME OF THE JOB when a determiner opens its phrase — the / a / an
//     / this / that / its / their / whole / overall / entire / total / full / any, then at most four words, none of them
//     the part ("the repair", "a moderate front-corner repair", "the panel repair itself", "the repair cost") — and no
//     "of / to / on / for" + the part follows within four words ("the repair of the bonnet" is the bonnet's). Otherwise
//     the noun is BOUND ("bonnet (repair)", "the bonnet repair", "Repair: bonnet").
// The clause contradicts the row only if at least one word is BOUND. A job-noun leaves the listed/bracketed part names as
// the job's content, which asserts no action. Fails toward the old rule: anything it cannot read as a job-noun is bound.
const _JOB_DET = /(?:^|[\s(])(?:the|a|an|this|that|its|their|whole|overall|entire|total|full|any)\s+((?:[\w’'-]+\s+){0,4})$/i;
function _actionBoundToPart(clause, name, kind) {
  const word = kind === 'repair' ? /\brepair(ed|s|ing)?\b/gi : /\breplac(e|ed|es|ing|ement|ements)\b/gi;
  for (const m of clause.matchAll(word)) {
    const suffix = (m[1] || '').toLowerCase();
    const isNoun = kind === 'repair' ? (suffix === '' || suffix === 's') : (suffix === 'ement' || suffix === 'ements');
    if (!isNoun) return true;
    const det = clause.slice(0, m.index).match(_JOB_DET);
    // a verb inside the phrase means the noun is an object ("the wing needs repair"), not the head of a job phrase
    if (!det || _mentionsPart(name, det[1]) || /\b(?:is|are|was|were|be|been|being|needs?|needed|requires?|required|wants?|gets?|got|under\w*)\b/i.test(det[1])) return true;
    const after = clause.slice(m.index + m[0].length).match(/^\s+(?:of|to|on|for)\s+((?:[\w’'-]+\s*){1,4})/i);
    if (after && _mentionsPart(name, after[1])) return true;
  }
  return false;
}
export { _actionBoundToPart as actionBoundToPartForTest };   // batch 191: validate-batch191 pins the rule
...
const _DRIVER_RX = [
  ...
  /\bcost drivers? (?:are|is)\b/i,   // batch 189 P2: "cost driver is" (singular) as well
...
//   / driven by / made up of …", or (batch 189 P2) the driver-list phrasing "whose cost driver(s) is/are …".
//   The head is kept word for word, cut at the word before "whose" and closed with "."; the code sentence follows it.
...
const _SHAPE_A_RX = /^(The band position\b.*?)\s+whose\s+(?:(?:costs?|repair)\s+(?:is|are)\s+(?:dominated by|driven by|made up of)|cost drivers?\s+(?:is|are))\b/i;
...
export function bindClaimClasses(text, ctx, mode = 'redflags') {
  ...
  // batch 187 P1: nothing is re-joined. A line with no dropped sentence is kept byte-identical; a dropped sentence is cut
  // out of the ORIGINAL line with the whitespace that separated it from its neighbour, so the text between the kept
  // sentences (spaces, bullet markers) is exactly what the model wrote.
  const keptLines = [];
  let edited = false;
  for (const line of text.split('\n')) {
    ...
    const spans = _sentenceSpans(body);
    const kept = [];
    const hits = [];
    for (const sp of spans) {
      const hit = _sentenceContradicts(sp.text, ctx, mode);
      // batch 175: no part-status class any more, so no clause drop (batch 171 P4's dropDemotedClauses is gone).
      if (hit) { hits.push({ class: hit.class, reason: hit.reason, sentence: sp.text }); sp.drop = true; continue; }
      kept.push(sp.text);
    }
    dropped.push(...hits);
    if (!hits.length) { keptLines.push(line); continue; }   // unedited line: byte-identical
    edited = true;
    if (kept.length === 0) { keptLines.push(null); continue; } // whole line was contradictory → drop the line
    ...
    // Rebuild from the original: the text before the first sentence, then each kept sentence preceded (after the first)
    // by the original gap that sat just before it, then the text after the last sentence.
    let b = body.slice(0, spans[0].start);
    let first = true;
    spans.forEach((sp, i) => {
      if (sp.drop) return;
      if (!first) b += body.slice(spans[i - 1].end, sp.start);
      b += body.slice(sp.start, sp.end);
      first = false;
    });
    b += body.slice(spans[spans.length - 1].end);
    keptLines.push(prefix + b);
  }
  if (!edited) return { text, dropped };                     // nothing dropped: the field comes back exactly as given
  // A dropped line takes its newline with it; where it sat between two blank lines, one of the blanks goes too.
  const lines = [];
  for (let i = 0; i < keptLines.length; i++) {
    const l = keptLines[i];
    if (l !== null) { lines.push(l); continue; }
    const prevBlank = lines.length && !lines[lines.length - 1].trim();
    const nextBlank = keptLines[i + 1] != null && !keptLines[i + 1].trim();
    if (prevBlank && nextBlank) i++;
  }
  let out = lines.join('\n');
  if (text === text.trim()) out = out.trim();                // the parser hands fields trimmed; keep them that way
