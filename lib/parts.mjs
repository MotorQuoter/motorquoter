...
export function codeOwnDriverSentence(text, chargedRows) {
  ...
  for (const sent of _splitSentences(text)) {
    ...
    if (kind === 'mixed') { held.push(sent); continue; }
    if (kind !== 'driver') continue;
    const i = text.indexOf(sent);
    if (i < 0) continue;
    const after = DRIVER_SENTENCE(chargedRows);
    ...
    return {
      ...
      stamp: { before: removed, after, rows: (chargedRows || []).map((r) => ({ panelId: r.panelId ?? null, name: r.name, action: r.action ?? null })) },
      held,
    };
  }
