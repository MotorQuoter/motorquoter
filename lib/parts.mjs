...
import { REPAIR_NO_PART_NOTE, isNonPartRow } from './labour.mjs';
...
export function assembleVdsParts(costedLedger, gatedParts) {
  ...
  for (const [i, gp] of (gatedParts || []).entries()) {
    ...
    const prose  = gp._repairNoPart ? REPAIR_NO_PART_NOTE
      ...
      : gp._srsTier && figure
      ? `${verb} — from ${figure}.`   // batch 129: the SRS kit figure is an approximate FROM figure (spec §10)
...
export function assembleKcdParts(gatedParts) {
  ...
  return (gatedParts || [])
    ...
    .map(gp => ({ panelId: gp.panelId ?? null, partName: gp.name, action: gp.action ?? 'replace', figure: gp.used ?? gp.oem ?? null, _from: !!gp._srsTier }))
    .filter(r => r.figure != null && r.figure > 0)
    .sort((a, b) => b.figure - a.figure)
    // batch 129: the SRS kit figure is an approximate FROM figure (spec §10); every other driver prints as before.
