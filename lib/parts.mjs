...
export function applyVisibilityGate(reconciledParts, costedParts, flaggedParts, lampResult) {
  ...
  for (const v of gateStripped) {
    ...
    console.log(`[GATE] stripped "${v.partName}" zone=${v.zone} ${ivLabel}${v._probeStripped ? ' [probe-stripped]' : ''}`);
    if (!flaggedParts.some(f => normName(f.partName) === normName(v.partName) && f._gateGenerated)) {
      const reason = v._seamModeStripped
        ? 'Excluded from repair total — the adjacent bumper was removed by tension (peel), which exposes the normal body seam without transmitting crush into this panel; the apparent crease is consistent with the exposed seam, not confirmed impact deformation — verify in person before bidding.'
        : v._probeStripped
          ? 'Excluded from repair total — flagged as costed by the primary assessment but not confirmed by an independent second review of the same photos; inspect this panel in person before bidding.'
          : v._apertureStripped
            ? 'Excluded from repair total — an independent blind review of the photos identified the visible feature as an exposed body seam or construction fold uncovered by the displaced bumper, not impact deformation of the panel itself; inspect in person before bidding.'
      ...
      flaggedParts.push({
        ...
        reason,
        _gateGenerated: true,
        ...(v._seamModeStripped  ? { _seamModeStripped: true }  : {}),
        ...(v._probeStripped     ? { _probeStripped: true }     : {}),
        ...(v._apertureStripped  ? { _apertureStripped: true }  : {}),
      });
    }
  }
