...
export function reconcileParts(parts, lampResult, costedParts = [], grilleAllowance = 0, bandKey = null, specLampBand = null, specLampAssumed = false, ledFallbackBand = null) {
  ...
  const insertedRow = (ordinal = null) => ({
  ...
  if (lampCount === 2) {
    ...
    if (lampIndices.length >= 1) {
      ...
      if (others.length >= 1) {
        ...
        workParts = workParts.map((item, i) => i === secondIdx ? mandate(item, cost1, band, keptOrdinal) : item);
        ...
        workParts = [...workParts.slice(0, at), insertedRow(keptOrdinal), ...workParts.slice(at)];
      ...
      const labourIdx = workParts.findIndex(p => /labour|paint|prep/i.test(p.name));
      const at = labourIdx >= 0 ? labourIdx : workParts.length;
      workParts = [...workParts.slice(0, at), insertedRow(), insertedRow(), ...workParts.slice(at)];
