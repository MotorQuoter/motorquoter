...
export function applyVisibilityGate(reconciledParts, costedParts, flaggedParts, lampResult) {
  ...
  // Ordinal pairing for G-split instances: k-th rawParts row for panelId X → k-th costedParts entry.
  // Array.find always returns the first match, so without this the second instance never gets its verdict.
  const _panelVerdicts = new Map(); // panelId → ordered costedParts entries
  const _panelOrdinals = new Map(); // panelId → consumed count
  for (const cp of costedParts) {
    if (cp.panelId == null) continue;
    if (!_panelVerdicts.has(cp.panelId)) _panelVerdicts.set(cp.panelId, []);
    _panelVerdicts.get(cp.panelId).push(cp);
  }
  for (const rp of reconciledParts) {
    ...
    const verdict = (rp._lampMandated || rp._lampOrdinal != null)
      ...
      : (() => {
          if (rp.panelId == null) return null;
          const arr = _panelVerdicts.get(rp.panelId) || [];
          const ord = _panelOrdinals.get(rp.panelId) ?? 0;
          _panelOrdinals.set(rp.panelId, ord + 1);
          return arr[ord] ?? arr[0] ?? null;
        })();
    if (rp._lampMandated) {
    ...
    if (verdict.independentlyVisible === true) {
      const vKey = verdict._instanceKey ?? normKey(verdict.partName);
      if (verdict._instanceKey) console.log(`[GATE][INSTANCE] "${rp.name}" instanceKey=${verdict._instanceKey} → dedup key="${vKey}"`);
