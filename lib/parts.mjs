...
export function assembleVdsParts(costedLedger, gatedParts) {
  ...
  for (const [i, gp] of (gatedParts || []).entries()) {
    ...
    const prose  = gp._repairNoPart ? REPAIR_NO_PART_NOTE
      ...
      : gp._srsFloor && figure
      ? `${verb} — from ${figure}.`   // batch 130: the SRS figure is the flat £500 FLOOR (spec §10)
...
export function assembleKcdParts(gatedParts) {
  ...
  return (gatedParts || [])
    ...
    .map(gp => ({ panelId: gp.panelId ?? null, partName: gp.name, action: gp.action ?? 'replace', figure: gp.used ?? gp.oem ?? null, _from: !!gp._srsFloor }))
