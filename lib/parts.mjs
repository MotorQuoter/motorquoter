...
export function reconcileParts(parts, lampResult, costedParts = [], grilleAllowance = 0, bandKey = null, specLampBand = null, specLampAssumed = false) {
  if (!lampResult?.tier2Fired || !lampResult.lampAllowance) {
    ...
    if (orphanLampIdx.length > 0) {
      if (specLampBand == null) {
        console.error(`[LAMP ORPHAN] ${orphanLampIdx.length} headlamp line(s) on tier-1 lot but specLampBand unavailable — left as model figure; investigate.`);
      } else {
        const orphanSet = new Set(orphanLampIdx);
        workParts = parts.map((p, i) => {
          if (!orphanSet.has(i)) return p;
          const modelCost = p.used ?? p.oem ?? 0;
          console.log(`[LAMP ORPHAN] "${p.name}" clamped model £${modelCost} → band £${specLampBand}`);
          return { ...p, used: specLampBand, oem: null, _lampMandated: true, _band: specLampBand, _modelLampCost: modelCost,
...
export function applyVisibilityGate(reconciledParts, costedParts, flaggedParts, lampResult) {
  ...
  for (const rp of reconciledParts) {
    ...
    if (rp._lampMandated) {
      ...
      console.log(`[GATE][LAMP] "${rp.name}" iv≠true — band £${rp._band} retained in repair total`);
...
// contradiction. Every detector fails OPEN (any doubt → keep). Code-owned lines (exemptPrefixes)
// pass through untouched. Returns { text, dropped:[{class}] } for loud logging.
...
function _sentenceContradicts(sentence, ctx, mode) {
  ...
  if (ctx.lampType && /\b(head\s?lamp|headlight|lamp unit|xenon|hid|led|halogen)\b/i.test(s)) {
    ...
    if (asserted && asserted !== ctx.lampType) return 'lamp-type';
  }
  ...
  if (mode === 'redflags' || _CLAIM_REPAIR_CTX.test(s)) {
    ...
    for (const f of figs) {
      if (Number.isFinite(f) && !ctx.allowedFigures.some(a => Math.abs(a - f) <= 1)) return 'figure';
    }
  }
  ...
  for (const [name, action] of ctx.partActions) {
    ...
    if (new RegExp(`\\b${_escapeRx(name)}\\b`, 'i').test(s)) {
      ...
      if (saysRepair && !saysReplace && action === 'replace') return 'action';
      if (saysReplace && !saysRepair && action === 'repair')  return 'action';
    }
  }
  ...
  for (const name of ctx.demoted) {
    if (name && new RegExp(`\\b${_escapeRx(name)}\\b`, 'i').test(s)
      && /\b(damag|costl?y|cost driver|biggest|expensive|replac|repair|write[- ]?off|structural)\b/i.test(s)) return 'part-status';
  }
  ...
  if (ctx.evVerdict && /\b(battery|hv|high[- ]voltage|traction (?:pack|battery)|ev[- ]system)\b/i.test(s)) {
    ...
    if (ctx.evVerdict === 'cost-prohibitive' && saysIntact) return 'ev';
    if (ctx.evVerdict === 'clear'            && saysDead)   return 'ev';
  }
  return null;
}
export function bindClaimClasses(text, ctx, mode = 'redflags', exemptPrefixes = []) {
  const dropped = [];
  ...
  for (const line of text.split('\n')) {
    ...
    if (exemptPrefixes.some(p => p && bare.startsWith(p.slice(0, 40)))) { keptLines.push(line); continue; }
    ...
    for (const sent of _splitSentences(body)) {
      const cls = _sentenceContradicts(sent, ctx, mode);
      if (cls) { dropped.push({ class: cls, sentence: sent.slice(0, 60) }); continue; }
      kept.push(sent);
    }
