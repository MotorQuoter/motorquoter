...
export function reconcileParts(parts, lampResult, costedParts = [], grilleAllowance = 0) {
...
// One block per repair line item actually in the total (gatedParts → action + the
// finalised band-derived figure), then one block per floored/flagged ledger panel
// carrying the code-owned reason already on the ledger entry (DISAGREE / NOT_VISIBLE /
// FLAG_CLASS / COSMETIC / APERTURE_*). Floored panels carry the reason ONLY — never a
// damage description. CLEAR panels are omitted: a confirmed-undamaged panel has no
// damage block and no flag. Returns [{ panelId, partName, action, prose }].
//
// Sources are the FINALISED ledger: gatedParts (post reconcile + visibility gate, so
// the figure matches the repair total) and flaggedLedger (post bumper-off demotion +
// post-call aperture-reason set). Reads no model prose.
export function assembleVdsParts(costedLedger, flaggedLedger, gatedParts) {
  const out  = [];
  const seen = new Set();
  const nameKey = s => 'name:' + normName(s || '');
  ...
  // 1. One block per real repair line item — action + finalised figure. Skip labour/paint.
  for (const gp of (gatedParts || [])) {
    ...
    if (gp.panelId != null) seen.add(gp.panelId);
    seen.add(nameKey(gp.name));
  }

  // 2. One block per floored/flagged panel not already costed — code-owned reason, no figure.
  for (const fp of (flaggedLedger || [])) {
    const k1 = fp.panelId, k2 = nameKey(fp.partName);
    if ((k1 != null && seen.has(k1)) || seen.has(k2)) continue;
    if (k1 != null) seen.add(k1);
    seen.add(k2);
    out.push({ panelId: fp.panelId ?? null, partName: fp.partName, action: null, prose: fp.reason || '' });
  }
...
export function buildBuyerFlags(assessment) {
  ...
  if (preGate.length === 0) return all;
  const preGatePanelIds = new Set(preGate.map(p => p.panelId).filter(Boolean));
  return all.filter(f => !f._amalgDisagree || preGatePanelIds.has(f.panelId));
}
