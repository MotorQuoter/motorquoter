...
export function applyVisibilityGate(reconciledParts, costedParts, flaggedParts, lampResult) {
  ...
  for (const rp of reconciledParts) {
    ...
    if (rp._lampMandated) {
      ...
      if (rp._lampPair && lampResult?.tier2Fired && lampResult.lampCount === 2) {
...
// A headlamp row, however it got there: code-owned (_lampMandated), named like a lamp, or keyed to the
// HEADLAMP panel. Deliberately wider than _lampMandated — this is what the counter must SEE, not what
// the band owns. Widening "mandated" instead would hide an unowned row rather than count it.
const isLampMoneyRow = p => p?._lampMandated === true || isLampLine(p?.name || '') || p?.panelId === PANEL.HEADLAMP;
...
export function classifyLampMoneyRows(gatedParts, lampResult, spanSource) {
  ...
  if (unowned.length > 0) {
    const figs = unowned.map(p => `"${p.name}" £${p.used ?? p.oem ?? 0}`).join(', ');
    return { level: 'warn', line: `[LAMP MONEY][NOT BAND-OWNED] ${rows.length} lamp row(s) in parts_sum, ${unowned.length} NOT band-owned (${figs}) — INVARIANT BROKEN: the band owns every headlamp pound; tier2=${!!lampResult?.tier2Fired} lampCount=${lampResult?.lampCount ?? 'n/a'}; span_source=${spanSource}.` };
  }
  const mandated = rows;
  ...
  return { level: 'warn', line: `[LAMP MONEY][ORPHAN COLLAPSE] ${n} lamp rows in parts_sum — tier2-anomaly (INVARIANT BROKEN: ${n} mandated lamp rows on a lampCount=${lampCount} path; the only sanctioned >1 case is the batch-109C full-width pair, exactly 2 rows both _lampPair); span_source=${spanSource}.` };
}
