  ...
  for (const v of gateStripped) {
    ...
    if (!flaggedParts.some(f => normName(f.partName) === normName(v.partName) && f._gateGenerated)) {
      const reason = v._bumperOffStripped
        ? 'Excluded from repair total — the adjacent bumper is displaced or torn away, exposing the body seam; the apparent crease on this panel is consistent with the exposed seam, not confirmed impact deformation — verify in person before bidding.'
        : v._probeStripped
