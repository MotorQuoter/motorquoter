...
import { PANEL, PANEL_BEHAVIOUR, PANEL_CLASS, EV_PANEL_RESOLVED_CLASS } from './panelEnum.mjs';
import { isStructureFloorPanel } from './structureFloor.mjs';
import { rowKeyFor } from './ledgerEdits.mjs';
import { REPAIR_NO_PART_NOTE, isNonPartRow, isFromFigureRow, Q4_DECLINED_CHECKLIST_ITEM, isBodyPanel } from './labour.mjs';
...
export function seedChecklistFromFlags(checklistText, buyerFlags, { lampTier2Fired = false } = {}) {
  ...
  for (const flag of buyerFlags || []) {
    ...
    if (flag._amalgDisagree) {
      ...
      seedItem = `Show ${part} close-up — the listing photographs disagree on this part; condition unconfirmed.`;
      ...
      seedItem = `Show ${part} close-up — not clear from the listing photographs; condition unconfirmed.`;
    } else if (flag._gateGenerated) {
      seedItem = `Show ${part} close-up — could not be confirmed from the listing photographs.`;
      ...
      seedItem = `Show ${part} close-up — inspection item; confirm condition before bidding.`;
      ...
      seedItem = `Show ${part} close-up — condition could not be confirmed from the listing photographs.`;
    }
    text += `\n${nextItem}. ${seedItem}`;
...
const inMoney = (r) => ((r?.used ?? r?.oem ?? 0) > 0) || r?._repairNoPart === true;
/**
...
// ── batch 151 V1 — A MAJORITY-DAMAGED DISPUTED PANEL IS COSTED ──────────────────────────────────────
// Vincent, 15 Sep: the every-panel losing-vote rule is DROPPED; disputed non-quarter panels stay COSTED with
// the note. The gate only ever kept a disputed panel when the MODEL wrote a Parts row for it (the ledger
// preamble tells the model FLOORED = "do NOT cost", so it usually does not) — CK75ONW's tailgate,
// BOOT_LID {views:3, damaged:2, clean:1, branch:"disagree"}, ended as a £0 flag with no row at all.
// This costs such a panel at its band figure when damaged > clean. The row is NOT a £0-rule row (no
// _zeroRule): the grade then owns repair/replace and panel work exactly as for any other costed panel
// (lib/labour.mjs applyGradeOwnsAction). The disagree flag is KEPT and marked so the buyer still sees it.
// Out of scope by ruling: the REAR QUARTER (its Q4 losing-vote rule, lib/labour.mjs); panels already in
// the money (a model row the gate kept, or any code-owned row); ties and minorities (report only).
export const DISAGREE_MAJORITY_EXCLUDED = Object.freeze(['REAR_QUARTER']);
/**
 * @param flags       coreObs.flaggedParts (the disagree flags; marked in place when costed)
 * @param costedParts coreObs.costedParts (the disagree twin carries the grade)
 * @param pvVotes     pvResult.pvVotesMap
 * @param gatedParts  the ledger so far (read only — the caller pushes the returned rows)
 * @returns { rows, skipped } — rows to push; skipped = [{ panelId, why }] for the log
 */
export function disagreeMajorityRows({ flags, costedParts, pvVotes, gatedParts, bandKey, table = PANEL_PRICE_TABLE, display = {} }) {
  const rows = [];
  const skipped = [];
  const inMoneyIds = new Set((gatedParts || []).filter(inMoney).map((r) => r.panelId).filter(Boolean));
  const disagreeCount = new Map();
  for (const f of (flags || [])) if (f?._amalgDisagree && f.panelId) disagreeCount.set(f.panelId, (disagreeCount.get(f.panelId) || 0) + 1);
  for (const f of (flags || [])) {
    if (!f?._amalgDisagree || !f.panelId) continue;
    const pid = f.panelId;
    const skip = (why) => skipped.push({ panelId: pid, why });
    if (DISAGREE_MAJORITY_EXCLUDED.includes(pid)) { skip('rear quarter — Q4 rule owns it'); continue; }
    if (PANEL_BEHAVIOUR[pid] !== PANEL_CLASS.COST) { skip('not a COST-class panel'); continue; }
    if (inMoneyIds.has(pid)) { skip('already in the money'); continue; }
    if (disagreeCount.get(pid) !== 1) { skip('more than one disagree flag for this panel — vote match ambiguous'); continue; }
    const v = pvVotes?.[pid];
    if (!v || v.branch !== 'disagree') { skip(`no disagree vote entry (branch=${v?.branch ?? 'absent'})`); continue; }
    if (!(v.damaged > v.clean)) { skip(`damaged ${v.damaged} ≤ clean ${v.clean} — not a majority (report only)`); continue; }
    const te = bandKey ? table?.[pid]?.[bandKey] : null;
    if (!te) { skip(bandKey ? `no table entry for band "${bandKey}"` : 'no band'); continue; }
    const twin = (costedParts || []).find((cp) => cp.panelId === pid && cp._amalgDisagree);
    const grade = twin?._ledgerSeverity || 'MODERATE';
    // A body panel's grade picks repair/replace (applyGradeOwnsAction then prices it). Glass, lamps, wheels and
    // other non-body parts have no repair path in the table — the band figure is a part — so they are replaced.
    const action = isBodyPanel(pid) ? (grade === 'SEVERE' ? 'replace' : 'repair') : 'replace';
    rows.push({ panelId: pid, name: display[pid] || f.partName || pid, action,
      oem: te.oem, used: te.used, _tableMandated: true, _disagreeCosted: true, _disagreeMajority: true,
      _votes: { damaged: v.damaged, clean: v.clean }, _grade: grade });
    f._disagreeMajorityCosted = true;
    inMoneyIds.add(pid);
  }
  return { rows, skipped };
}
...
export function buildBuyerFlags(assessment) {
  ...
  const disagreeFiltered = preGate.length === 0
    ...
    : all.filter(f => {
        ...
        // batch 151 V1: a disputed panel CODE costed on a damaged majority keeps its note — it is in the money
        // without a model row, so it is absent from _preGateParts by construction.
        const keep = !f._amalgDisagree || f._q4Declined === true || f._disagreeMajorityCosted === true || preGatePanelIds.has(f.panelId);
        if (!keep) {
