...
export function applyVisibilityGate(reconciledParts, costedParts, flaggedParts, lampResult) {
  ...
  const blockAbsent = costedParts.length === 0 && nonAllowanceParts.length > 0;
  if (blockAbsent) {
    console.error(
  ...
  for (const rp of reconciledParts) {
    if (rp._allowance) { gatedParts.push(rp); continue; }
    const verdict = (rp._lampMandated || rp._lampOrdinal != null)
      ? lampVerdictFor(costedParts, rp.name, rp._lampOrdinal ?? null)
      : costedParts.find(cp => normName(cp.partName) === normName(rp.name)) ?? null;
    if (rp._lampMandated) {
      ...
      if (!flaggedParts.some(f => normName(f.partName) === normName(rp.name))) {
        flaggedParts.push({
          partName: rp.name, zone: 'front', weight: 'medium',
          reason: `model's lamp pricing not independently confirmed — precautionary £${rp._band} (${lampType}) allowance retained in repair total; confirm serviceable unit on inspection`,
          _gateGenerated: true,
        });
      }
      continue;
    ...
    if (verdict.independentlyVisible === true) { gatedParts.push(rp); continue; }
