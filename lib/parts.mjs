...
export function reconcileParts(parts, lampResult, costedParts = [], grilleAllowance = 0, bandKey = null, specLampBand = null, specLampAssumed = false, ledFallbackBand = null) {
  ...
  if (lampCount === 2) {
    ...
    //
    // NOT CHANGED: the gate's A1 rule (Vincent, 5 Aug) still applies to both rows together — if the
    // paired verdict is iv≠true, BOTH lamps move to the inspection allowance, exactly as one did
    // before. This ruling stops the SECOND lamp being singled out; it does not overrule A1.
