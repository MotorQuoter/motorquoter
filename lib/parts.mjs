...
export function discloseSplitVoteUncosted({ pvVotes, gatedParts, flags, display = {} }) {
  ...
  for (const [pid, v] of Object.entries(pvVotes || {})) {
    ...
    const reason = SPLIT_VOTE_REASON(partName);
...
export function seedChecklistFromFlags(checklistText, buyerFlags, { lampTier2Fired = false } = {}) {
  ...
  for (const flag of buyerFlags || []) {
    ...
    if (flag._splitVoteUncosted === true) {
      ...
      const line = aperture ? flag.reason : SPLIT_VOTE_REASON(part);
...
// batch 171 P4 (Vincent, 21 Sep) — A PART-STATUS CLAIM DROPS THE CLAUSE, NOT THE SENTENCE.
// CK75ONW run 2 lost its whole Visible Damage Summary synthesis because one clause named the (probe-floored) front
// bumper — and the trigger word ("biggest") sat in a DIFFERENT clause, about the rear. Split the sentence on "; ", " — ",
// ", plus " and ", and "; drop every clause that names a demoted part; keep the rest if the rebuilt sentence contradicts
// nothing (any class) and still says something (≥ 4 words). Otherwise the whole sentence goes, as before.
const _CLAUSE_SPLIT = /(\s+—\s+|;\s+|,\s+plus\s+|,\s+and\s+)/;
export function dropDemotedClauses(sentence, ctx, mode = 'redflags') {
  const parts = String(sentence || '').split(_CLAUSE_SPLIT);
  if (parts.length < 3) return null;                                   // one clause — nothing to keep apart
  const clauses = [{ delim: '', text: parts[0] }];
  for (let i = 1; i < parts.length; i += 2) clauses.push({ delim: parts[i], text: parts[i + 1] ?? '' });
  const names = (ctx?.demoted || []).filter(Boolean);
  const kept = clauses.filter((c) => !names.some((n) => _mentionsPart(n, c.text)));
  const gone = clauses.filter((c) => !kept.includes(c));
  if (!gone.length || !kept.length) return null;
  let out = kept.map((c, i) => (i === 0 ? '' : c.delim) + c.text).join('').trim();
  out = out.replace(/[,;—\s]+$/, '');
  if (!/[.!?]$/.test(out)) out += '.';
  out = out.charAt(0).toUpperCase() + out.slice(1);
  if (out.split(/\s+/).filter(Boolean).length < 4) return null;       // nothing meaningful left
  if (_sentenceContradicts(out, ctx, mode)) return null;               // still contradicts → the whole sentence goes
  return { text: out, droppedClauses: gone.map((c) => c.text.trim()) };
}
...
export function bindClaimClasses(text, ctx, mode = 'redflags') {
  ...
  for (const line of text.split('\n')) {
    ...
    for (const sent of _splitSentences(body)) {
      ...
      if (hit && hit.class === 'part-status') {
        const partial = dropDemotedClauses(sent, ctx, mode);            // batch 171 P4
        if (partial) {
          for (const c of partial.droppedClauses) hits.push({ class: 'part-status-clause', reason: hit.reason, sentence: c });
          kept.push(partial.text);
          continue;
        }
      }
