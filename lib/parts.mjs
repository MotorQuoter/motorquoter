...
// of the checklist on the grounds that this curated entry covers them. Returned only on tier 2 with a lamp
// actually in the money. ONE string — verdictLine / costDriverEntry / tier1Line stay dark.
export function lampChecklistItem(lampResult, gatedParts) {
  if (!lampResult?.tier2Fired || !lampResult.checklistEntry) return null;
  if (!(gatedParts || []).some(p => p._lampMandated)) return null;
