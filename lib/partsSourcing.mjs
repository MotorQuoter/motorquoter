...
// Rows that are not shoppable objects — labour, the SRS fitting rider, allowances and the jig floor are
// operations, not parts. batch 117: the ONE owner is lib/labour.isNonPartRow (markers first, the model-labour
// name test only as the fallback) — the old name-only regex let "SRS fitting" through as a buy-on-eBay part.
import { isNonPartRow } from './labour.mjs';
...
export function buildPartsSourcing({ parts, vehicle, epn } = {}) {
  ...
  for (const p of parts) {
    ...
    if (!name) continue;
    if (isNonPartRow(p)) continue;                   // labour / SRS fitting / allowance / floor — not shoppable

    const key = name.toLowerCase();
    if (seen.has(key)) {                             // one link per distinct part — but remember EVERY ledger
      const prior = out.links.find(l => l.part.toLowerCase() === key);   // row it stands for (batch 117)
      if (prior && p._rowKey) prior._rowKeys.push(p._rowKey);
      continue;
    }
    seen.add(key);
    ...
    out.links.push({
      ...
      tracked,
      // batch 117: the ledger row keys this link stands for (two identical fog lamps → one link, two keys), so
      // the buyer edit layer can drop it once EVERY row it covers is struck. Empty when the caller passed none.
      _rowKeys: p._rowKey ? [p._rowKey] : [],
    });
  }
  return out;
}
