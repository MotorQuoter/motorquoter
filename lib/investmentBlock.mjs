...
export function buildInvestmentBlock(input = {}) {
  ...
  if (isNum(resale) && feeStackFn) {
    if (vatQualifying) {
      // batch 141 item 2: the hammer at which hammer + 20% hammer VAT + fees = the resale figure.
      const value = ceilingHammerForOutlay(resale, feeStackFn, { vatQualifying: true });
      flip = { value, assumption: `Buy, no repair, resell as salvage ≈ £${r0(resale)}; less VAT on the hammer and buyer fees — zero margin. Take off the profit you want.` };
    } else {
      const f = fees(resale);
      const value = Math.max(0, r0(resale - f));
      flip = { value, assumption: `Buy, no repair, resell as salvage ≈ £${r0(resale)}; less buyer fees — zero margin. Take off the profit you want.` };
    }
  }
  ...
  if (partOut && isNum(partOut.low) && feeStackFn) {
    ...
    if (vatQualifying) {
      // batch 141 item 2: the hammer at which hammer + 20% hammer VAT + fees = recovery less dismantling.
      const value = ceilingHammerForOutlay(recovery - DISMANTLING_ALLOWANCE, feeStackFn, { vatQualifying: true });
      partsOut = { value, assumption: `Conservative parts recovery £${r0(recovery)}; less £${DISMANTLING_ALLOWANCE} dismantling, VAT on the hammer and buyer fees.` };
    } else {
