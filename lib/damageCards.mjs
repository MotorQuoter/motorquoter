...
import { REPAIR_NO_PART_NOTE, STRUCT_FLOOR_NOTE, isNonPartRow, isFromFigureRow } from './labour.mjs';
...
export function buildDamageCards({ gatedParts = [], costedParts = [], flaggedParts = [], allowanceParts = [], limitFlags = null } = {}) {
  ...
  // batch 142 R3 (Vincent, 16 Sep): when the costed "SRS airbag kit (deployed) — from £500" card is
  // present, the second "SRS airbag (deployed) — Related, inspect: £0" card is NOT shown. They are the
  // same subject twice: one says the kit is in the repair total, the other reads as an uncosted extra.
  // The name dedup below could never catch it — norm() strips only the bracket, leaving "srs airbag kit"
  // against "srs airbag". Keyed on the MARKERS instead (_srsFloor on the costed row, route.js SRS_INJECT;
  // _srsExtentFloor on the flag), so it can never drift with the wording. The FLAG itself is untouched:
  // it still renders in Inspection Flags, carrying the same sentence, and still seeds the checklist.
  const srsCosted = gatedParts.some(gp => gp?._srsFloor);
  ...
  for (const gp of gatedParts) {
    ...
    cards.push({
      ...
      ...(isFromFigureRow(gp) ? { _fromFigure: true } : {}),   // batch 131: one owner — a floor card renders "from £X" (spec §5/§10)
    });
  ...
  for (const f of flaggedParts) {
    ...
    if (srsCosted && f?._srsExtentFloor) continue;     // batch 142 R3 — the costed SRS card already says this
    cards.push({
      ...
      // batch 143 T2 (Vincent, 16 Sep — "flags only"): an UNCOSTABLE £0 line — the resolving === 0
      // not-visible branch (route.js) — is marked so the two Damage Breakdown renderers can drop it.
      // The card is NOT removed from _damageCards: lib/flooredProseScrub.mjs derives its FLOORED panel
      // set from this array, and without the card the model could assert damage on that panel in Key
      // Cost Drivers / Visible Damage Summary with nothing left to scrub it. Display filter, not a
      // data deletion — see the T2 note in the handoff.
      ...(f?._amalgNotVisible ? { _notVisibleFloor: true } : {}),
    });
  }
