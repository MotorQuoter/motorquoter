...
// Wheel and tyre share a 4-state OBSERVATION-CONFIDENCE vocabulary (06 Jun fix — SR16GOT
// slot/prose clash: the old intact/kerbed/damaged/not-shown set had no truthful state for
// "no dedicated close-up, but it's plainly visible in the general shots and looks fine" — so
// the model recorded "not-shown" in the slot while truthfully writing "all four alloys and
// tyres appear intact" in prose. The 4 states name exactly what the model can conclude, so
// slot and prose answer the SAME question and land on the SAME answer: dedicated-photo-intact /
// no-dedicated-shot-but-appears-intact / damaged / genuinely-not-visible.
export const VERDICT_VOCAB = {
  confirmation: ['confirmed', 'unconfirmed', 'discrepancy'],
  damage:       ['damaged', 'undamaged', 'not-visible'],
  wheel:        ['dedicated-photo-intact', 'no-dedicated-shot-but-appears-intact', 'damaged', 'genuinely-not-visible'],
  tyre:         ['dedicated-photo-intact', 'no-dedicated-shot-but-appears-intact', 'damaged', 'genuinely-not-visible'],
};

// Verdicts that collapse a slot into the "Verified clear" confirmation block.
// Deliberately narrow: genuinely-not-visible / no-dedicated-shot-but-appears-intact / not-visible /
// unconfirmed are information ("here's what we could and couldn't see — check on inspection"),
// never silently folded into all-clear alongside a genuine dedicated-shot clean read.
export const ALL_CLEAR_VERDICTS = {
  confirmation: ['confirmed'],
  damage:       ['undamaged'],
  wheel:        ['dedicated-photo-intact'],
  tyre:         ['dedicated-photo-intact'],
};
